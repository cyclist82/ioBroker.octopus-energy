...
class OctopusEnergy extends utils.Adapter {
   ...
   */
  async updateEpexAnalysis(baseId, prices) {
    var _a, _b, _c, _d;
    try {
      ...
      const cur = (_a = slots.find((s) => now >= s.start && now < s.end)) != null ? _a : slots[slots.length - 1];
      const cheapestWindow = (windowSlots) => {
        let bestAvg = Infinity;
        let bestStart = null;
        for (let i = 0; i <= slots.length - windowSlots; i++) {
          const windowAvg = slots.slice(i, i + windowSlots).reduce((sum, s) => sum + s.ct, 0) / windowSlots;
      ...
      const fmtTime = (d) => d ? d.toLocaleTimeString("de-DE", {
        hour: "2-digit",
        minute: "2-digit",
        timeZone: "Europe/Berlin"
      }) : "";
      const round4 = (n) => Math.round(n * 1e4) / 1e4;
      await this.setState(`${baseId}.epexCurrentPrice`, { val: round4((_b = cur == null ? void 0 : cur.ct) != null ? _b : 0), ack: true });
      ...
      await this.setState(`${baseId}.epexIsCheapNow`, { val: ((_c = cur == null ? void 0 : cur.ct) != null ? _c : 0) <= avg, ack: true });
      ...
      this.log.debug(
        `EPEX analysis: now=${round4((_d = cur == null ? void 0 : cur.ct) != null ? _d : 0)} ct/kWh, avg=${round4(avg)}, cheap1h=${fmtTime(cheapestWindow(4))}, cheap4h=${fmtTime(cheapestWindow(16))}`
      );
    } catch (error) {
